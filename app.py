...
page = st.sidebar.radio(
    "Go to",
    ["🏠 Upload & Rank Resumes", "📄 View Resumes", "🗑 Delete One", "🧹 Delete All"],
)
...
if page == "🏠 Upload & Rank Resumes":
    ...
    if uploaded_files and job_description:
        ...
        for i in range(len(file_names)):
            ...
            if existing:
                st.warning(f"⚠ {file_names[i]} already exists.")
...
# ------------------ Delete One Resume ------------------
elif page == "🗑 Delete One":
    st.title("🗑 Delete a Specific Resume")
...
# ------------------ Delete All Resumes ------------------
    ...
    st.warning("⚠ This will permanently remove all resumes from the database.")

    confirm = st.checkbox("I understand and want to delete all resumes")

    if confirm and st.button("❌ Confirm Delete All"):
        deleted = collection.delete_many({})
        st.success(f"🗑 Deleted {deleted.deleted_count} resumes.")